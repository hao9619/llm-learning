...
<|im_start|>assistant
LoRA 用 FP16 加载基座，QLoRA 先把基座量化到 4bit……<|im_end|>
<|im_start|>assistant
--- 输入结束 ---
末尾的 <|im_start|>assistant 是「生成提示符」，模型从这里往下续写。
（此刻没有待回答的提问，所以它紧跟在上一条回答后面；真正提问时，你的问题会插在这两者中间。）
```

末尾出现两个连着的 `<|im_start|>assistant` 是正常的：
`/prompt` 本身不是提问，所以历史停在助手的回答上，
`add_generation_prompt=True` 又在后面补了一个生成提示符。
真正提问时，你的问题会插在这两者中间。

如果这里能看到完整历史，说明上下文送到了。
那模型还是答得不好，就是模型自身的能力问题，
而不是上下文管理的问题 —— 怎么把这两件事分清楚，见下一节。
...
现象是：连聊几轮之后问模型「我刚刚问你什么问题？」，
它回答「看起来我们之前的对话可能中断了，你刚刚提问的内容我已经记不清楚了」。
看起来就是上下文没送进去。

结论先说：**代码没有问题，是问法的问题。**
下面三步任何时候都能照着走一遍。

### 第一步：用 `/prompt` 看模型到底收到了什么

这是唯一能看到原始输入的地方，别靠猜。三件事一眼就能确认：

1. **历史在不在** —— 早几轮的消息还能不能看到
2. **有没有被截断** —— 看它报的 token 数离 `max_context_tokens` 还有多远，
   到顶了就是 `trim()` 把最旧的几轮丢掉了，这属于预期行为，调大预算即可
3. **角色有没有错位** —— `user` / `assistant` 是不是严格交替

上面那次排查里，`/prompt` 显示 381 / 2048 token，
被问到的那句话完整地躺在输入里。**第一步就排除了链路问题。**

### 第二步：拿字数除以 token 数，验证特殊标记没被切碎

`<|im_start|>` 这类标记在词表里各占 **1 个 token**。
但推理脚本用的是慢速 tokenizer（`use_fast=False`），
万一它没把这些标记当特殊标记、而是按普通文字逐字切开，
每个就会膨胀到 5~8 个 token —— ChatML 的结构也就废了，
模型看到的不再是「对话」，而是一堆夹着乱码的散文。

这种情况不会报错，但用除法一算就现形：

```text
正文 613 字 ÷ 381 token ≈ 1.6 字/token   ← 中文的正常比例，标记是对的
正文 613 字 ÷ 510 token ≈ 1.2 字/token   ← 明显偏低，标记被切碎了
```

那次实测是 1.6，而且输入里含 21 个特殊标记，
如果它们各膨胀 6 倍，总数会多出一百多。**第二步排除了 tokenizer 问题。**

### 第三步：问内容引用，不要问元问题

前两步都过了，就该怀疑问法本身了。这是这次排查最值得记下来的一条：

| 问法 | 例子 | 能不能用来测上下文 |
|---|---|---|
| **元问题** | 「我刚刚问你什么问题？」 | ❌ 不能 |
| **内容引用** | 「我多大了？」（年龄在前几轮说过） | ✅ 能 |

元问题问的是**对话本身**，不是对话内容。
3B 这个量级的模型对这类问题天然就弱，
很容易触发「对话可能中断了」这套客套模板 —— 哪怕历史就明明白白摆在它眼前。
用它来测上下文，测的其实是模型的元认知能力，不是上下文管道。

内容引用就干净多了：答案只存在于早几轮的历史里，
答对了，就只可能是从上下文读到的，没有别的解释。

换成内容引用之后，基座和 LoRA 都答对了：
...
用户：等，我几岁阿？
助手：你30岁了，不算很年轻，但已经有很多工作和生活经验了。
```

| 脚本 | 结果 |
|---|---|
| `chat_qwen.py`（基座） | ✅ 答对 |
| `infer_lora_qwen.py`（LoRA） | ✅ 答对 |

两边都正常，**上下文管理模块工作正常，代码不需要改**。

### 附带的一个结论：LoRA 没有洗掉多轮能力

排查中一度怀疑：训练脚本构造的样本全是单轮
（`build_prompt` / `format_example` 都只产出 system + user + assistant 三条），
LoRA 是不是从没见过多轮输入、因而只会盯着最后一句答？

实测推翻了这个猜想。原因是 LoRA 只改动少量低秩权重，
基座本身的多轮对话能力被保留了下来。
所以训练数据是单轮的，**不代表**微调后的模型就只能单轮对话。
